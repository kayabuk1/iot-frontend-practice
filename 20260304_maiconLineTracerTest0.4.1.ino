...
<2026-03-04log>
仮◆エラー9：カーブのきつい場所が曲がり切れず復帰動作が挟まる。
仮◆エラー10：復帰動作が挟まる時、後退⇒ごく僅かに首振り⇒再度後退⇒ごく僅かに首振りを繰り返してしまう。後退⇒ある程度首振りでコースをもう少し捉えたい。
原因？：曲がる時の右輪と左輪の速度差が小さく曲がり切れていない。#define MAX_SPEED 120
対処案： int speed_up = map(l_val,0,1023,HALF_SPEED,MAX_SPEED);
　　　　 int speed_down = map(l_val,0,1023,HALF_SPEED,★64);//反比例させる。
        map()関数、第4引数を255*0.25=63.75≒64に設定。
        ⇒コンデンサーで電圧安定効果もあり、カーブ曲がりやすくなったが、まだカーブで復帰動作が挟まるときがある。
        ⇒コンデンサーでの電圧安定効果もあるのでさらにgradu関数出力下限値を下げて動けないか検討。255*0.20=51にでテスと。
        ⇒map(l_val,0,1023,HALF_SPEED,51)
        ⇒モーターがストール状態になることはない。数回復帰動作は挟まるが緩やかなカーブ、トンネル区間、連続カーブは走行出来ている。
        if (val_L < WHITE  && val_C < WHITE && val_R < WHITE){
          delay(20);
 ...
 他
  for(int i = 0; i < 50; i++)
  {
    ...
    delay(10);
        ...
        ⇒comeback関数の判定条件を厳しくする修正で、一時OKとし、残り時間を鑑みて他を優先することに。

◆エラー11：ほかの人のモータードライバー焼き切れ故障頻発
対処：モーターに0.1μFのコンデンサーをはんだ付け。さらにコンデンサーをモータ躯体にボディアースとしても接続。
    　またモータードライバーVm⇔GND間に470μF電解コンデンサを追加（急停止急発進時の安定出力＆ノイズ対策）
結果：モータの出力安定化？カーブやや滑らかに曲がれている様に見える。
◆エラー12：モータードライバーマジックスモーク
原因：放課後自習時に疲れて、プラスとマイナス取り違え。
対処：報告後、新しいモータードライバーに交換。
